...
/// 局部重绘编辑器窗口标签（🔴 2026-08-21：聊天缩略图编辑，单例）
const IMAGE_EDITOR_LABEL: &str = "image-editor";

/// 编辑器图片暂存桶写入（🔴 2026-08-21）。
/// 壳前端跨域 fetch gateway 会失败（gateway CORS 仅画布同源），
/// 故经 Rust reqwest 代理（与壳其他 gateway 通信一致，无 CORS 问题）。
#[tauri::command]
pub async fn editor_result_set(
    app: tauri::AppHandle,
    ...
    let gw = format!("http://127.0.0.1:{}/v1/images/editor-result", port);
    let body = serde_json::json!({ "session_id": session_id, "url": url });
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| format!("POST editor-result failed: {}", e))?;
    if !resp.status().is_success() {
        // 🔴 诊断：附带响应体 + 端口来源对比（残留旧进程时 used_port ≠ state_file_port）
        let status = resp.status();
        let body_text = resp.text().await.unwrap_or_default();
        let state_file_port: String = {
            let home = crate::resolve_eleve_home();
            let f = home.join("runtime").join("gateway_state.json");
        ...
        return Err(format!(
            "POST editor-result HTTP {}: {} (used_port={} state_file_port={})",
            status,
...
/// 编辑器图片暂存桶读取（🔴 2026-08-21，一次性消费）
#[tauri::command]
pub async fn editor_result_get(
    app: tauri::AppHandle,
    state: tauri::State<'_, crate::TauriAppState>,
    session_id: String,
    ...
    let port = crate::resolve_gateway_port_cached(&app, &state).await?;
    let gw = format!(
        "http://127.0.0.1:{}/v1/images/editor-result?session_id={}",
        port, session_id
    );
    let client = reqwest::Client::new();
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| format!("GET editor-result failed: {}", e))?;
    let data: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| format!("parse editor-result failed: {}", e))?;
    Ok(data
        .get("url")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string()))
...
/// 打开聊天缩略图的局部重绘编辑器窗口（2026-08-21）。
/// 协议（零跨 WS 依赖，gateway editor-result 端点作 session 级图片暂存桶）：
/// 1. 壳先 POST 原图 data URL → /v1/images/editor-result（sessionId 作 key）
/// 2. 本命令只传 sessionId 开窗（URL 短，data URL 不进 query）
/// 3. 编辑器页 GET 取原图 → 涂抹 → 确认 POST 标注图覆盖
/// 4. 壳监听窗口关闭 → GET 取回标注图 → 作为聊天附件
#[tauri::command]
pub async fn open_image_editor_window(
    app: tauri::AppHandle,
    state: tauri::State<'_, crate::TauriAppState>,
    session_id: String,
) -> Result<String, String> {
    // 单例但每次全新内容：已存在 → 销毁重建（编辑器 URL 带 sessionId，
    // show 旧窗口不刷新会显示上一张图；destroy 触发壳的 onCloseRequested，
    // 那次 GET 结果为空（取走即删）无害）
    if let Some(w) = app.get_webview_window(IMAGE_EDITOR_LABEL) {
        let _ = w.destroy();
    }
    let port = crate::resolve_gateway_port_cached(&app, &state).await?;
    let url = format!(
        "http://127.0.0.1:{}/plugins/canvas/index.html?editor=1&sessionId={}",
        port, session_id
    );
    let builder = tauri::WebviewWindowBuilder::new(
        &app,
        IMAGE_EDITOR_LABEL,
        tauri::WebviewUrl::External(url.parse().map_err(|e| format!("editor url invalid: {}", e))?),
    )
    .title("局部重绘编辑")
    ...
    let w = builder.build().map_err(|e| {
        tracing::error!("[CANVAS] image editor window create FAILED: {}", e);
        format!("image editor window create failed: {}", e)
    })?;
    let _ = w.show();
    let _ = w.set_focus();
    tracing::warn!("[CANVAS] image editor window created (port {})", port);
