...
export default function MessageBubble({ type, content, streaming, onRegenerate, onEdit, agentAttribution }: MessageBubbleProps) {
  ...
  if (type === 'user') {
    ...
    return (
      <div className="bg-primary text-primary-foreground rounded-2xl rounded-br-sm px-3 py-2 text-sm max-w-[80%] ml-auto relative group">
  ...
  if (streaming) {
    return (
      <div className="bg-card text-card-foreground rounded-2xl rounded-bl-sm px-3 py-2 text-sm border border-border shadow-sm">
  ...
  return (
    <div className="bg-card text-card-foreground rounded-2xl rounded-bl-sm px-3 py-2 text-sm border border-border shadow-sm">
      ...
      <span ref={textRef} className="prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: renderedHtml || '<em>(无内容)</em>' }} />
