...
interface TextTabMetaProps extends React.ComponentPropsWithoutRef<'span'> {}

function TextTabMeta({ className, ...props }: TextTabMetaProps) {
  return <span className={cn('text-[0.72em] font-normal text-(--ui-text-tertiary)', className)} {...props} />
}

interface TextTabProps extends React.ComponentPropsWithoutRef<'button'> {
  active?: boolean
}
