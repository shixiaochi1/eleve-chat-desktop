...
type BaseCurveOptions = Pick<LoaderCurve, 'durationMs' | 'particleCount' | 'pulseDurationMs' | 'trailSpan'> & {
  rotate?: boolean
  rotationDurationMs?: number
  strokeWidth?: number
  point?: (progress: number, detailScale: number) => Point
...
const LOADER_CURVES: Record<string, LoaderCurve> = {
  ...
  'cardioid-glow': cardioidCurve('Cardioid Glow', {
    a: 8.4,
    particleCount: 72,
    pointFor(t: number, r: number, scale: number) {
      return {
        x: 50 + Math.cos(t) * r * scale,
        y: 50 + Math.sin(t) * r * scale
      }
    },
    rFor(t: number, a: number) {
        ...
        <g ref={groupRef}>
          ...
          {Array.from({ length: config.particleCount }, (_, index) => (
            <circle
              fill="currentColor"
              key={`${type}-${index}`}
              ref={(node: SVGCircleElement | null) => {
