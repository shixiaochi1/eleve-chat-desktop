/**
 * ImageEditorModal v9 — 聊天区图片编辑（🔴 2026-08-29 定稿：红框标注合成图）
 *
 * 用户拍板（2026-08-29）：编辑器 = 纯标注合成工具——涂抹 → 合成"原图 + 红色粗边框"
 * 一张图（对齐画布重绘 overlay 语义：只边框零填充，原图像素不被污染）→ 替换原图
 * 进附件 → 用户在输入框打提示词 → 发给 ELEVE。
 * ELEVE 侧：agent 收到合成图 + 用户提示词 → 调 image_generate（image_url = 合成图，
 * prompt = 按工具描述模板扩写）→ 走图生图链路出图（比例严格 = 原图）。
 * v8 的"编辑器内直调 /v1/images/inpaint"已撤销——agent 在中间只做填参数，
 * 生图统一走 image_generate 工具。
 */
...
export function ImageEditorModal({ src, name, onConfirm, onCancel }: ImageEditorModalProps) {
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      // 2. 🔴 v9（2026-08-29 用户拍板）：红框描边（对齐画布重绘 overlay 语义）——
      //    环带 = 膨胀(mask, k) 挖掉 mask 本体 → 只画红色粗边框，区域内零填充
      //    （半透明红填充会污染原图像素——改材质/颜色时模型恰需看到原图细节）
      const k = Math.max(6, Math.round(Math.max(w, h) / 160)) // 描边宽度随图幅缩放
      const ring = document.createElement('canvas')
      ring.width = w
      ring.height = h
      const rc = ring.getContext('2d')!
      // 2a. 膨胀：mask 按 8 邻域 + 4 正交方向偏移 k 各画一次（并集 ≈ 膨胀 k px）
      const offsets: Array<[number, number]> = [
        [-k, 0], [k, 0], [0, -k], [0, k],
        [-k, -k], [k, -k], [-k, k], [k, k],
      ]
      for (const [dx, dy] of offsets) rc.drawImage(maskLayer, dx, dy)
      // 染红（255,60,60 对齐画布语义）
      rc.globalCompositeOperation = 'source-in'
      rc.fillStyle = 'rgb(255,60,60)'
      rc.fillRect(0, 0, w, h)
      // 2b. 挖掉 mask 本体 → 留下外圈环带（区域内像素原样，零污染）
      rc.globalCompositeOperation = 'destination-out'
      rc.drawImage(maskLayer, 0, 0)
  ...
  const btn = (t: ToolMode, label: string, title: string, active: boolean, onClick: () => void) => (
    <button
      onClick={onClick}
      title={title}
      className={`rounded-md px-2.5 py-1.5 text-xs transition-colors ${active ? 'bg-primary text-primary-foreground' : 'bg-white/10 text-white hover:bg-white/25'}`}
    >
      {label}
    </button>
  )
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-black/90 backdrop-blur-sm" role="dialog" aria-modal="true">
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-t border-white/10 bg-black/40 flex-wrap">
        ...
        <button onClick={() => setShowMask(v => !v)} title="蒙版预览（红色标注/白色蒙版切换）" className={`rounded-md px-2.5 py-1.5 text-xs transition-colors ${showMask ? 'bg-success text-white' : 'bg-white/10 text-white hover:bg-white/25'}`}>
          👁 蒙版{showMask ? '：白' : '：红'}
        </button>
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-t border-white/10 bg-black/40">
        ...
        <div className="ml-auto flex gap-2">
          ...
          <button onClick={handleConfirm} disabled={saving} className="rounded-lg px-6 py-2 text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
            {saving ? '合成中...' : '✓ 确认'}
          </button>
        </div>
      </div>
