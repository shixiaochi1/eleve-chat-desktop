// eleve-chat-desktop/src/store/terminal-buffer.ts
/**
 * 对齐 Hermes apps/desktop/src/app/right-sidebar/terminal/buffer.ts
 * 
...
/**
 * Build a reader from an xterm Terminal instance.
 * 对齐 Hermes makeTerminalReader(): 逐行调 translateToString(true) 获取纯文本，
 * right-trim + resolve wide chars + drop SGR colors.
 */
export function makeTerminalReader(term: Terminal): Reader {
  return ({ start, count }) => {
    ...
    // Strip trailing blank lines (对齐 Hermes)
