...
### 丁-5 处置（2026-09-17 已落地）：统一标记 + 死代码门禁

老大拍板「**加 `@deprecated 未挂载` 标记**，看板现在只有左侧边栏的，原先的独立看板先留着」
⇒ 22 个孤儿**全部保留**，改为「**登记 + 门禁**」治理。

**1) 统一标记（22 个文件，插在文件首）**

每个文件头部插入同一模板，逐文件定制 4 个字段；标记串固定为 `🔴 @deprecated 未挂载（unreachable）`：

| 字段 | 内容 |
|---|---|
| 是什么 | 组件/模块一句话 + 行数 |
| 性质 | `功能孤儿（级联链头 / 级联）` 或 `设计系统预留组件` |
| 消费者 | 无（全仓零 import）／仅某孤儿文件（其自身不可达） |
| 继承者 | 替代者（如 `SidebarKanbanBoard` / `ProjectTreePanel`）；**无**时显式写明功能缺口 |
| 背景 | 死因提交（如 `88a0fcc` 删独立会话按钮 / `51b9ba4` 看板迁侧栏） |

统一尾注：
> ⚠️ 请勿在此处修补 bug 或新增功能 —— 本文件无消费者，改动对运行时零影响
> （历史上已多次出现「给孤儿打补丁」）。要复活请先接回入口；确认废弃则连同其下游一起删，勿只删链头。

用 `@deprecated`（而非单纯注释）的理由：TS/编辑器会**给 import 它的代码标删除线**，链内互引也随之提示 —— 天然的「别在这儿改」护栏。

**2) 死代码门禁 `scripts/deadcode-check.mjs`（新增，零依赖 Node ESM）**

> ⚠️ 本仓原本**没有 eslint 配置、没有 CI、没有 lint 脚本** —— "门禁"此前并不存在。
> 本次把它实体化：`npm run deadcode`。

- **① 文件级**：从 `src/main.tsx` BFS 可达性（含 `@/` 别名与 index 解析，排除 `*.test.*` / `*.d.ts`）
- **② 标记核对**：`未登记孤儿`（不可达 + 无标记）❌ / `标记过期`（可达 + 有标记）⚠️ —— 两类都判失败（exit 1）
- **③ 符号级**（`--symbols`，可选）：统计 `tsc --noUnusedLocals` 告警数（不作为失败项）

**3) 门禁经过反向验证**（三个分支都实测，用临时 probe 文件，测完已清理）

| 场景 | 期望 | 实测 |
|---|---|---|
| 新建未标记孤儿 | 抓出 + exit 1 | ✅ `未登记的死文件 1 个` / exit 1 |
| 给它加标记 | 豁免 + exit 0 | ✅ `已登记孤儿 23 个` / exit 0 |
| 把它接回入口（标记仍在） | 报「标记过期」 | ✅ `标记过期 1 个` / exit 1 |

**当前门禁输出**：生产源文件 366 · 可达 344 · 已登记孤儿 22（≈5285 行含标记）· 未登记 0 · **通过 ✅**
